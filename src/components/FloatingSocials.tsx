...
export const FloatingSocials: React.FC = () => {
  ...
  return (
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-40 hidden sm:flex flex-col gap-2 pointer-events-auto items-end">
      {socials.map((social) => (
        <a
          key={social.name}
          href={social.url}
          ...
          className={`group relative flex items-center justify-end w-11 hover:w-36 h-11 bg-white border-y border-l border-slate-200/90 rounded-l-2xl shadow-md hover:shadow-xl transition-all duration-300 ease-out overflow-hidden ${social.color} ${social.hoverBg}`}
        >
          ...
          <span className="absolute left-3.5 font-black text-xs tracking-wider opacity-0 -translate-x-3 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 delay-75 ease-out whitespace-nowrap pointer-events-none z-10">
            {social.name}
          </span>

          {/* Social Logo in stationary fixed position — spins 360 on its center axis */}
          <div className="w-11 h-11 flex items-center justify-center flex-shrink-0 z-10">
